...
always_print_full_model_comment <- TRUE # suggerito = TRUE se si sostituiscono regressori


datapartenza <- c(2024,7)

# old models
#mynamesmens <- c("FATEXP_10", "FATEXP_11", "FATEXP_13", "FATEXP_14", "FATEXP_15", "FATEXP_16", "FATEXP_17", "FATEXP_18", "FATEXP_19", "FATEXP_20", "FATEXP_21", "FATEXP_22", "FATEXP_23", "FATEXP_24", "FATEXP_25", "FATEXP_26", "FATEXP_27", "FATEXP_28", "FATEXP_29", "FATEXP_31", "FATEXP_32", "FATEXP_33", "FATEXP_C", "C_DEFL", "DIVIZ08", "DIVID08", "DIVIE08", "DIVIZ10", "DIVID10", "DIVIE10", "DIVIZ11", "DIVID11", "DIVIE11", "DIVIZ13", "DIVID13", "DIVIE13", "DIVIZ14", "DIVID14", "DIVIE14", "DIVIZ15", "DIVID15", "DIVIE15", "DIVIZ16", "DIVID16", "DIVIE16", "DIVIZ17", "DIVID17", "DIVIE17", "DIVIZ18", "DIVID18", "DIVIE18", "DIVID19", "DIVIZ20", "DIVID20", "DIVIE20", "DIVIZ21", "DIVID21", "DIVIE21", "DIVIZ22", "DIVID22", "DIVIE22", "DIVIZ23", "DIVID23", "DIVIE23", "DIVIZ24", "DIVID24", "DIVIE24", "DIVIZ25", "DIVID25", "DIVIE25", "DIVIZ26", "DIVID26", "DIVIE26", "DIVIZ27", "DIVID27", "DIVIE27", "DIVIZ28", "DIVID28", "DIVIE28", "DIVIZ29", "DIVID29", "DIVIE29", "DIVIZ30", "DIVIZ31", "DIVID31", "DIVIE31", "DIVIZ32", "DIVID32", "DIVIE32", "DIVIZ33", "DIVID33", "MIGSZ_1", "NONDUNAZ", "NONDUEST", "ENERGNAZ", "ENERGEST", "MIGSZ_2", "DUREVNAZ", "DUREVEST", "MIGSZ_3", "INVESNAZ", "INVESEST", "MIGSZ_4", "INTERNAZ", "INTEREST") #,"DIVIZ06", "DIVID06", "DIVIE06", "DIVIZ09", "DIVID09", "DIVIE09", "DIVIZ12", "DIVID12", "DIVIE12", "DIVIZ19", "DIVIE19", "DIVID30", "DIVIE30", "DIVIE33", "MIGSZ15")
mynamesmens <- c("FATEXP_10", "FATEXP_11", "FATEXP_13", "FATEXP_14", "FATEXP_15", "FATEXP_16", "FATEXP_17", "FATEXP_18", "FATEXP_19", "FATEXP_20", "FATEXP_21", "FATEXP_22", "FATEXP_23", "FATEXP_24", "FATEXP_25", "FATEXP_26", "FATEXP_27", "FATEXP_28", "FATEXP_29", "FATEXP_31", "FATEXP_32", "FATEXP_33", "FATEXP_C", "DFSEZDC", "DFSEZEC", "DIVIZ06", "DIVID06", "DIVIE06", "DIVIZ08", "DIVID08", "DIVIE08", "DIVIZ09", "DIVID09", "DIVIE09", "DIVIZ10", "DIVID10", "DIVIE10", "DIVIZ11", "DIVID11", "DIVIE11", "DIVIZ12", "DIVID12", "DIVIE12", "DIVIZ13", "DIVID13", "DIVIE13", "DIVIZ14", "DIVID14", "DIVIE14", "DIVIZ15", "DIVID15", "DIVIE15", "DIVIZ16", "DIVID16", "DIVIE16", "DIVIZ17", "DIVID17", "DIVIE17", "DIVIZ18", "DIVID18", "DIVIE18", "DIVIZ19", "DIVID19", "DIVIE19", "DIVIZ20", "DIVID20", "DIVIE20", "DIVIZ21", "DIVID21", "DIVIE21", "DIVIZ22", "DIVID22", "DIVIE22", "DIVIZ23", "DIVID23", "DIVIE23", "DIVIZ24", "DIVID24", "DIVIE24", "DIVIZ25", "DIVID25", "DIVIE25", "DIVIZ26", "DIVID26", "DIVIE26", "DIVIZ27", "DIVID27", "DIVIE27", "DIVIZ28", "DIVID28", "DIVIE28", "DIVIZ29", "DIVID29", "DIVIE29", "DIVIZ30", "DIVID30", "DIVIE30", "DIVIZ31", "DIVID31", "DIVIE31", "DIVIZ32", "DIVID32", "DIVIE32", "DIVIZ33", "DIVID33", "DIVIE33", "MIGSZ_1", "NONDUNAZ", "NONDUEST", "MIGSZ15", "ENERGNAZ", "ENERGEST", "MIGSZ_2", "DUREVNAZ", "DUREVEST", "MIGSZ_3", "INVESNAZ", "INVESEST", "MIGSZ_4", "INTERNAZ", "INTEREST")
...
mens_present <- (!is.null(mynamesmens) && length(mynamesmens)>0)
trim_present <- (!is.null(mynamestrim) && length(mynamestrim)>0)
#se ci saranno sia modelli finali che trimestrali, verranno valorizzate (in automatico) queste variabili, altrimenti si valorizzerà l'unica variabile modelli_finali
modelli_finali_mens <-NULL
modelli_finali_trim <-NULL
...
if (file.exists('modellimens_nuovi.txt')){
  ...
  # converti tutti i nomi delle colonne in caps lock
  colnames(indici) <- toupper(colnames(indici))
  ...
  if(length(mynamesmens)>0){
    ...
    if(mens_present && trim_present){ # utile solo ai fini di preparare il file per JDemetra+ in caso di modelli sia trimestrali che mensili presenti
      write(tsinput,"seriemens.txt")
    }
    ...
    if(mens_present && trim_present){ # utile solo ai fini di preparare il file per JDemetra+ in caso di modelli sia trimestrali che mensili presenti
      write(tsinput,"serienewmens.txt")
    }
    ...
    # Sincronizza gli elementi presenti in vecchi$nomi ma non in nuovi$nomi e viceversa, visto che alcuni modelli possono essere chiusi/aggiunti
    vecchi_armonizzati       <- synchronize_models(vecchi, nuovi)

    pdf("mens_new.pdf",paper='a4r',width=0)
    for (ii in 1:length(nuovi$nomi)){
      plot(nuovi,ii,vecchi_armonizzati)
    }
    dev.off()
...
if (file.exists('modellitrim_nuovi.txt')){
  ...
  if(length(mynamestrim)>0){
    ...
    if(mens_present && trim_present){ # utile solo ai fini di preparare il file per JDemetra+ in caso di modelli sia trimestrali che mensili presenti
      write(tsinput,"serienewtrim.txt")
    }
    ...
    for (ii in 1:length(nuovi$nomi)){
      #plot(nuovi,ii)
      plot(nuovi,ii,vecchi_armonizzati,quarterly = TRUE)
    ...
    # scrivi file modelli
    if (diffmod==TRUE){
      modelli_finali_trim <- diffmodelli(modelli_nuovi,modelli_orig)
    } else {
      modelli_finali_trim <- modelli_nuovi
    }

    if(length(mynamesmens)>0 && !is.null(mynamesmens)){ # se ci sono modelli mensili già aggiunti a modelli finali
      modelli_finali_mens <- modelli_finali
      modelli_finali <- NULL
    }
    else{
      modelli_finali <- modelli_finali_trim
      modelli_finali_trim <- NULL
    }
...
if (!is.null(modelli_finali_trim)){
  modelli_finali_trim$ANNO_INI <- datapartenza[1]
  modelli_finali_trim$MESE_INI <- datapartenza[2]
  write.table(modelli_finali_trim,"modelli_finali_trim.txt",quote=F,col.names=F,row.names=F,sep=";",na="null")
}
if (!is.null(modelli_finali_mens)){
  modelli_finali_mens$ANNO_INI <- datapartenza[1]
  modelli_finali_mens$MESE_INI <- datapartenza[2]
  write.table(modelli_finali_mens,"modelli_finali_mens.txt",quote=F,col.names=F,row.names=F,sep=";",na="null")
}


if(trim_present && mens_present)
{
...
}else # presenti solo modelli mensili o solo modelli trimestrali
{
  ...
  if ( nrow(nuovi$diagmat) >0)
  {
    ...
    for(m_or_t in series_periods)
    {
      ...
      for(titolo in nuovi$diagmat$TITLE)
      {
        ...
        if(length(easter_variations)==0)
        {
          easter_variations = FALSE
        }
        ...
        # }
        if(nrow(old_model)>0 && old_model$EE==1) # stampa la Pasqua sempre, se c'?
        {
          easter_effect_old = df_calend_old[df_calend_old$TITLE==titolo,]$params_string
          easter_effect_old = get_EE(easter_effect_old)
          comment = paste0(comment, "###  EE:" , easter_effect_old, "\n")
...
files_to_send_common <- c("elaborazione.out", "elaborazione_commented.out", "modelli_finali.txt")



if(mens_present){
  files_to_send <- c("sa_new.csv", "newm/toutlier.m", "confronto_mens_orig_nuovi.pdf", "modellimens_nuovi.txt", "mens_new.pdf", "modelli_finali_mens.txt")
  files_to_send <- c(files_to_send, c("newm/tcalend.m", "newm/tregvar.m", "newm/tarmapar.m", "newm/ttest.m"))
}
if(trim_present){
  if(mens_present){
    files_to_send_t <- c("sa_new_trim.csv" , "newt/toutlier.m", "confronto_trim_orig_nuovi.pdf", "modellitrim_nuovi.txt", "trim_new.pdf", "modelli_finali_trim.txt")
    files_to_send_t <- c(files_to_send_t, c("newt/tcalend.m", "newt/tregvar.m", "newt/tarmapar.m", "newt/ttest.m"))
  } else{ # only trim model
    files_to_send <- c("sa_new_trim.csv" , "newt/toutlier.m", "confronto_trim_orig_nuovi.pdf", "modellitrim_nuovi.txt", "trim_new.pdf")
    files_to_send <- c(files_to_send, c("newt/tcalend.m", "newt/tregvar.m", "newt/tarmapar.m", "newt/ttest.m"))
  }
}

if(!(trim_present && mens_present)){ # solo un tipo di modelli presente fra mens e trim
  files_to_send <- c(files_to_send_common, files_to_send)
  make_files_to_send_folder(folder_name = "toSend" , to_send = files_to_send, begin_cleaning_folder = TRUE)
}else{ # both trim and mens present
  make_files_to_send_folder(folder_name = "toSend" , to_send = files_to_send_common, begin_cleaning_folder = TRUE)
  make_files_to_send_folder(folder_name = "toSend/trim" , to_send = files_to_send_t, begin_cleaning_folder = TRUE)
  make_files_to_send_folder(folder_name = "toSend/mens" , to_send = files_to_send  , begin_cleaning_folder = TRUE)

  aggiungi_suffisso_copia_liv_superiore("toSend/trim", "trim" ,"t")
  aggiungi_suffisso_copia_liv_superiore("toSend/mens", "mens" ,"m")
