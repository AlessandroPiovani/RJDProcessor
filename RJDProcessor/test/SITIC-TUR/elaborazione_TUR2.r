...
#perl <- "C:\\Perl\\Strawberry\\perl\\site\\bin"


#mynamesmens <- c("CL1042","CL1439")

# Questo 04 2022 - CL1042 CL1439
mynamesmens <- c("VATASA", "VATAIA", "VATASC", "VATAIC", "VATPSA", "VATPIA", "VATPSC", "VATPIC")


# mynamesmens <- c("CL2341")

a=1
#elenco CLASSI escluse: coincidenti con DIVI e GRU; CLA2443 diffusa in forma grezza;
#NB:CL2732 non più coincidente con GRU273 perché dal 2010 subentrata altra CL2731 tuttavia quasi coincidenti quindi eliminata dalla lista sotto e poi hanno ancora stesso modello
#NB:CL3011 non più coincidente con GRU301 perché dal 2015 subentrata altra CL3012 tuttavia quasi coincidenti quindi eliminata dalla lista sotto e poi hanno ancora stesso modello
#NB:CL3299 non più coincidente con GRU329 perché dal 2015 subentrata altra CL3291 tuttavia quasi coincidenti quindi eliminata dalla lista sotto e poihanno ancora stesso modello

mynamestrim <- NULL
datapartenza <- c(2022,3)
outputdata <- TRUE
diffmod <- TRUE
...
if (file.exists('modellimens_nuovi.txt')){
   ...
   if(length(mynamesmens)>0){
      ...
      if(Sys.info()["sysname"]!='Linux'){
        ...
        # vecchio system("C:\\Users\\UTENTE\\ProgramFiles\\SEATS\\Seats.exe -o newm -g newgm",ignore.stdout = TRUE,ignore.stderr = TRUE)
...
if (file.exists('modellitrim_nuovi.txt')){
   ...
   if(length(mynamestrim)>0){
      ...
      vecchi <- output_ts("oldt",freq=frequenza)
      nuovi <- output_ts("newt",freq=frequenza)
