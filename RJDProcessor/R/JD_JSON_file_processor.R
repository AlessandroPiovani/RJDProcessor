...
compare_sa_ts <- function(new_r_model=NA, new_model_workspace, old_model_workspace, materialized_ws_new=FALSE, materialized_ws_old=TRUE, java_processing_old_model=TRUE)
{
  ...
  pdf(file="comparisons.pdf", width = 11, height = 6)
  ...
  for(series in model_new[[1]])
  {
    ...
    # Allarga il margine destro

    y_range <- range(c(new_model_ts, old_model_ts), na.rm = TRUE)

    # Layout: 1 riga, 2 colonne
    layout(matrix(1:2, nrow = 1), widths = c(6, 1))

    # Primo pannello: grafico
    par(mar = c(5, 4, 4, 1))
    plot(new_model_ts, col = "blue", lty = 1, ylab = "Values", xlab = "Time",
         main = new_model_name, ylim = y_range)

    if (!is.null(old_models) && !all(is.na(old_models))) {
      lines(old_model_ts, col = "red", lty = 2)
    }

    # Secondo pannello: solo legenda
    par(mar = c(0, 0, 0, 0))
    plot.new()
    legend("center", legend = c("new model", "old model"),
